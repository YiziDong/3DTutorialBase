...
public class PlayerController : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        for (int i = 0; i < m_Attacks.Length; i++) {
            ...
            if (attack.WindUpTime > attack.FrozenTime) {
                Debug.LogError(attack.AttackName + "has a wind up time that is larger than the amount of time that the player is forzen for");
            }
                }
    }
    ...
    private void FixedUpdate()
    {
        ...
        if (p_Velocity.sqrMagnitude > 0) { // calculating the sqrmagnitude is faster than using magnitude
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (other.CompareTag("HealthPill")) {
            IncreaseHealth(other.GetComponent<HealthPill>().HealthGen);
